#!/usr/bin/env python3
"""Wired speaker controller for the cassette recorder network.

This variant keeps the performance layout, but monitor audio stays on each
Raspberry Pi: USB sound-card input is routed locally to the Pi/ALSA output.
"""
...
class RecorderDevice:
    ...
    last_seen: float = 0.0
    local_monitor_running: bool = False
    local_monitor_level: float = 0.0
    local_monitor_volume: float = 1.0
    local_monitor_muted: bool = False
...
class PerformanceCanvas:
    ...
    def draw_player(self, x1, x2, index, dev: RecorderDevice | None):
        ...
        self.text(left, 220, "PI LOCAL MONITOR", size=15)
        meter_x = right - 53
        self.vertical_meter(meter_x, 257, 441, self.gui.player_audio_level(dev))
        self.text(left, 260, "ROUTE", size=13, fill=MUTED)
        self.text(left, 286, "USB IN -> PI OUT", size=12, fill=GREEN)
    ...
    def draw_status_bar(self):
        ...
        self.text(27, 915, "CASSETTE WIRED SPEAKER CONTROLLER", size=13, anchor="w")
...
class PerformanceGUI:
    def __init__(self, root: tk.Tk, *, windowed: bool = True) -> None:
        ...
        self.root.title("Cassette Wired Speaker Controller")
        ...
        self.log("[INIT] Wired speaker GUI ready")
    ...
    def update_device_from_status(self, base_url, data):
        ...
        local_monitor = data.get("local_monitor")
        ...
        if isinstance(local_monitor, dict):
            dev.local_monitor_running = bool(local_monitor.get("running", False))
            dev.local_monitor_level = float(local_monitor.get("level", 0.0) or 0.0)
            dev.local_monitor_volume = float(local_monitor.get("volume", dev.monitor_volume) or 0.0)
            dev.local_monitor_muted = bool(local_monitor.get("muted", dev.monitor_muted))
            dev.monitor_volume = dev.local_monitor_volume
            dev.monitor_muted = dev.local_monitor_muted
    ...
    def sync_mixer_sources(self):
        self.sync_wired_monitors()

    def sync_wired_monitors(self):
        for dev in self.ordered_devices():
            should_run = dev.online and dev.mode == "play"

            if should_run and not dev.local_monitor_running:
                self.start_wired_monitor(dev)
            elif not should_run and dev.local_monitor_running:
    ...
    def local_monitor_query(self, dev):
        ...
        return (
            ...
            f"&input=auto&output={output_device}"
            f"&rate={AUDIO_RATE}&channels={AUDIO_CHANNELS}"
        )

    def start_wired_monitor(self, dev):
        self.request_async([dev.url], f"/audio/local-monitor/start?{self.local_monitor_query(dev)}")

    def stop_wired_monitor(self, dev):
        self.request_async([dev.url], "/audio/local-monitor/stop")

    def set_wired_monitor_volume(self, dev):
        self.request_async([dev.url], f"/audio/local-monitor/volume?{self.local_monitor_query(dev)}")
    ...
    def handle_drag(self, action, payload, x, rect, commit=False):
        ...
        if action == "monitor_volume_drag":
            ...
            if dev:
                ...
                dev.local_monitor_volume = dev.monitor_volume
                self.redraw()

                if commit:
                    self.set_wired_monitor_volume(dev)
    ...
    def handle_action(self, action, payload):
        ...
        if action == "scan":
            ...
            if dev:
                ...
                dev.local_monitor_muted = dev.monitor_muted
                self.set_wired_monitor_volume(dev)
                self.redraw()
    ...
    def mixer_status(self):
        if any(dev.local_monitor_running for dev in self.ordered_devices()):
            return "Pi monitor on"

        if self.playable_mixer_urls():
            return "Pi monitor starting"

        return "Pi monitor idle"
    ...
    def player_audio_level(self, dev):
        ...
        if dev.local_monitor_muted or dev.monitor_muted:
            return 0.0

        return clamp(dev.local_monitor_level * dev.monitor_volume, 0.0, 1.0)
...
def parse_args():
    parser = argparse.ArgumentParser(description="Run the cassette wired speaker controller.")
